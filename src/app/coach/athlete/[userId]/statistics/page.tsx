...
export default async function CoachAthleteStatisticsPage({
  ...
  const basePath = `/coach/athlete/${params.userId}/statistics`
  ...
  const [{ data: rangeActivities }, { data: zoneData }, { data: userSettingsData }, { data: illnessData }] = await Promise.all([
    ...
    db.from('illness_log').select('*').eq('user_id', targetUserId)
  ...
  const { data: intervalSetsRaw } = intervalActivityIds.length > 0
    ? await db.from('interval_sets').select('*').in('activity_id', intervalActivityIds).eq('user_id', targetUserId)
  ...
  return (
    <AppShell>
      ...
      <div className="flex items-end justify-between mb-6">
        ...
        <TimeRangeSelector current={range} offset={offset} periodLabel={label} basePath={basePath} />
      </div>
